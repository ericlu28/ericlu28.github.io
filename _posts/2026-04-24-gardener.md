---
title: "Building scheduled agents for continuous codebase health improvements"
date: 2026-04-24
...
We had a couple guiding questions:
...
2. And how do we make a dent in the 80% of engineering time that isn't coding: the meetings, debates, and analysis?
...
The more important design decision came first: keep the cron scheduling or workflow trigger out of the agent logic entirely. That means swapping the scheduler for a GitHub Actions workflow, a Slack slash command, or anything else is a simple change. The scheduling layer is intentionally thin.

With that constraint in mind, I evaluated three options: [**Cursor Automations**](https://cursor.com/docs/cloud-agent/automations), [**Claude Code Routines**](https://code.claude.com/docs/en/routines), and [**Jenkins**](https://www.jenkins.io/solutions/pipeline/).
...
Gardener runs scheduled agents via Jenkins cron jobs. Each agent scans a registered set of repos, finds one issue per run, and automatically opens a pull request with no human intervention required. We split the system into three phases: orchestration, deterministic agent enrichment, and the agent reasoning loop. The diagram below shows how these phases flow together.
...
Another thing- integrate this:
Stumbled on this paper that dives into Claude Code's architecture by reading its actual TypeScript source. Wild stat: only ~1.6% of the codebase is AI decision logic — the other 98.4% is permission gates, context compaction, subagent isolation, and extensibility plumbing (MCP/plugins/skills/hooks). Pretty useful if you're trying to figure out what a production agent harness actually looks like under the hood: https://arxiv.org/abs/2604.14228
